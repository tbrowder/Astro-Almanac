...
# p. 347
my $ra-m   = 134.6885; # degrees right ascension, Moon
...
my $cos-psi = sind($decl-s) * sind($decl-m) + cosd($decl-s) * cosd($decl-m) * cosd($ra-s - $ra-m); # Eq. 48.2
my $psi     = acosd $cos-psi;
my $tan-i   = ($R * sind($psi)) / ($D - $R * $cos-psi); # Eq. 48.3
my $i       = atand $tan-i;
my $k       = (1 + cosd($i)) / 2; # Eq. 48.1
my $tan-chi = (cosd($decl-s) * sind($ra-s - $ra-m))
             / (sind($decl-s) * cosd($decl-m) - cosd($decl-s) * sind($decl-m) * cosd($ra-s - $ra-m)); # Eq. 48.5
